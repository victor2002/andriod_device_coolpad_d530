    ...
    if (!strncasecmp(mMIME, "video/", 6)) {
        ...
        if (mIsEncoder) {
            ...
            if(!strcmp(mComponentName, "OMX.TI.DUCATI1.VIDEO.DECODER")) {
                ...
                /* Looking for SAR properties */
                if (meta->findInt32(kKeySARIdc, &sarIdc)) {
                    CHECK(meta->findInt32(kKeySARWidth, &sarWidth));
                    CHECK(meta->findInt32(kKeySARHeight, &sarHeight));
                } else {
                    sarIdc = SAR_IDC_UNSPECIFIED;
                }

                /* Updating output picture size if SAR defined */
                if (sarIdc != SAR_IDC_UNSPECIFIED) {
                    LOGI("Output picture width will be recalculated according to SAR");
                    width = (width * sarWidth) / sarHeight;
                }
            }
#endif
...
#if defined(OMAP_ENHANCEMENT)
    if(err != OK){
        LOGE("%s::Error allocating buffers", __FUNCTION__);
        return NO_MEMORY;
    }
#else
    CHECK_EQ(err, OK);
...
void OMXCodec::setBuffers(Vector< sp<IMemory> > mBufferAddresses, bool portReconfig){
    ...
    if(!portReconfig){
        ...
        while(mPortBuffers[kPortIndexOutput].size()) {
            usleep(2000); // 2 mS
            LOGD("(%d) Waiting for all buffers to be returned.. %d", retrycount, mPortBuffers[kPortIndexOutput].size());
            retrycount++;
