...
public final class CdmaCallTracker extends CallTracker {
    ...
    static final int MAX_CONNECTIONS = 2; //from 2.2  // only 1 connection allowed in CDMA
    static final int MAX_CONNECTIONS_PER_CALL = 2; //from 2.2 // only 1 connection allowed per call
    ...
    private static DriverCall CWCallDc = null;

    ////////////////////+
    //private static DriverCall CWCallDc = null;
    static final int DEFAULT_CW_ANSWER_PERIOD_MILLIS = 40000;

    private static int dcSize = 0;
    private static int mHoldNum = 0;
    private static boolean mRejectCWCall = false;
    CdmaCallWaitingNotification callwaitingNotification = null;
    RegistrantList fwimRegistrants = new RegistrantList();
    //RegistrantList gpsRegistrants = new RegistrantList();
    //RegistrantList secretRegistrants = new RegistrantList();
    CdmaConnection threeWayMO = null;
    ////////////////////////
    
    CdmaConnection pendingMO = null;
    boolean hangupPendingMO = false;
...
//    boolean needsPoll;
    ...
    CdmaCallTracker(CDMAPhone phone) {
        ...
        cm.registerForCdmaFwim(this, 19, null);
        //cm.registerForGpsNotifyRequest(this, 20, null);
        cm.registerForCdmaSecret(this, 21, null);
    ...
    public void dispose() {
        ...
        for(CdmaConnection c : connections) {
            try {
                if(c != null) hangup(c);
                //below code: concern: hung up long time
                //c.onDisconnect(com.android.internal.telephony.Connection.DisconnectCause.LOST_SIGNAL);
        ...
        try {
            ...
            //pendingMO.onDisconnect(com.android.internal.telephony.Connection.DisconnectCause.LOST_SIGNAL);
        } catch (CallStateException ex) {
            Log.e(LOG_TAG, "unexpected error on hangup during dispose");
        }
    ...
    ///////////+
    public void registerForFWIMReport(Handler handler, int i, Object obj)
    {
        Registrant registrant = new Registrant(handler, i, obj);
        fwimRegistrants.add(registrant);
    }

    //public void registerForGPSReport(Handler handler, int i, Object obj)
    //{
    //    Registrant registrant = new Registrant(handler, i, obj);
    //    gpsRegistrants.add(registrant);
    // }

    //public void registerForSecretReport(Handler handler, int i, Object obj)
    //{
    //    Registrant registrant = new Registrant(handler, i, obj);
    //    secretRegistrants.add(registrant);
    //}
    ...
    fakeHoldForegroundBeforeDial() {
        ...
        if (DBG_POLL) Log.d("CDMA", "CP_COMM fakeHoldForegroundBeforeDial");
    ...
    dial (String dialString, int clirMode) throws CallStateException {
        ...
        foregroundCall.setGeneric(false);
        if (DBG_POLL) Log.i("CDMA", (new StringBuilder()).append("dial: mHoldNum = ").append(mHoldNum).toString());//+
        if (DBG_POLL) Log.i("CDMA", (new StringBuilder()).append("dial: foregroundCall.getState() = ").append(foregroundCall.getState()).toString());//+
        if(mHoldNum <= 0)//+
            mHoldNum = 0;//+
        ...
        if (pendingMO.address == null || pendingMO.address.length() == 0
            ...
            if(!isPhoneInEcmMode || (isPhoneInEcmMode && isEmergencyCall)) {
                if(isEmergencyCall)
                {
                    if (DBG_POLL) Log.d("CDMA", "CP_COMM isEmergencyCall = "+pendingMO.address);
                    cm.dial(pendingMO.address, 9, obtainCompleteMessage());
                } else
                {
                    if (DBG_POLL) Log.d("CDMA", "CP_COMM isNormalCall = "+pendingMO.address);
                    if(dialString.charAt(dialString.length() - 1) == 'M')
                        cm.dial((new StringBuilder()).append(pendingMO.address).append('M').toString(), clirMode, obtainCompleteMessage());
                    else
    ...
    dialThreeWay (String dialString) {
        if (DBG_POLL) Log.i("CDMA", "dialThreeWay: foregroundCall.getState() = " + foregroundCall.getState() + "foreground Idle="+foregroundCall.isIdle());
    ...
    private void dumpState()
    {
        ...
        for(int j = list.size(); i < j; i++)
            Log.i("CDMA", list.get(i).toString());

        Log.i("CDMA", (new StringBuilder()).append("Foreground call: ").append(foregroundCall.toString()).toString());
        List list1 = foregroundCall.getConnections();
        int k = 0;
        for(int l = list1.size(); k < l; k++)
            Log.i("CDMA", list1.get(k).toString());

        Log.i("CDMA", (new StringBuilder()).append("Background call: ").append(backgroundCall.toString()).toString());
        List list2 = backgroundCall.getConnections();
        int i1 = 0;
        for(int j1 = list2.size(); i1 < j1; i1++)
            Log.i("CDMA", list2.get(i1).toString());
    ...
    {
    //    cm.requestSecrecyConnecting(null);
    }
    void sendCommandFromEngmode(String s)
    {
        if (DBG_POLL) log((new StringBuilder()).append("sendCommandFromEngmode (5) strcmd ").append(s).toString());
        //cm.sendCommandFromEngmode(s, null);
    }

    void sendGpsOneCommand(int i)
    {
        if (DBG_POLL) log((new StringBuilder()).append("[CdmaCallTracker] sendGpsOneCommand icmd: ").append(i).toString());
    //    cm.sendGpsOneCommand(i, null);
    }

    void setGpsNotifyResp(int i)
    {
        if (DBG_POLL) log((new StringBuilder()).append("[CdmaCallTracker] setGpsNotifyResp number =").append(i).toString());
        //cm.setGpsNotifyResp(i, null);
    }

    void setVoiceRecord(boolean flag)
    {
        if (DBG_POLL) log((new StringBuilder()).append("setVoiceRecord (1) enable: %d").append(flag).toString());
        //cm.setVoiceRecord(flag, null);
    }
    ...
    conference() throws CallStateException {
        ...
        //flashAndSetGenericTrue();
        mHoldNum = 0;
        if (DBG_POLL) Log.d("CDMA", (new StringBuilder()).append("CP_COMM conference() = ").append(mHoldNum).toString());
        cm.conference(obtainCompleteMessage(11));
    ...
    void
    clearDisconnected() {
        if (DBG_POLL) Log.d("CDMA", "CP_COMM clearDisconnected()");
    ...
    ///+
    public void clearForCdma()
    {
        if (DBG_POLL) Log.d("CDMA", "CP_COMM clearForCdma()");
        internalClearDisconnectedForCdma();
        updatePhoneState();
        phone.notifyPreciseCallStateChanged();
    }
    /////
    ...
    canDial() {
        ...
        if(!ret)
        {
            com.android.internal.telephony.CommandsInterface.RadioState radiostate = cm.getRadioState();
            if (DBG_POLL) Log.e("CDMA", (new StringBuilder()).append("CP_COMM: radioState = ").append(radiostate).append(" pendingMO = ").append(pendingMO).append(" ringingCall = ").append(ringingCall.isRinging()).append(" foregroundCall = ").append(foregroundCall.getState().isAlive()).append(" foregroundCall.getState = ").append(foregroundCall.getState()).append(" backgroundCall = ").append(foregroundCall.getState().isAlive()).toString());
        }
        return ret;
    }
    ...
    private void
    internalClearDisconnected() {
        if (DBG_POLL) Log.d("CDMA", "CP_COMM internalClearDisconnected()");
        ringingCall.clearDisconnected();
        foregroundCall.clearDisconnected();
        backgroundCall.clearDisconnected();
    }
    /////+
    private void internalClearDisconnectedForCdma()
    {
        if (DBG_POLL) Log.d("CDMA", "CP_COMM internalClearDisconnectedForCdma()");
        //ringingCall.clearDisconnectedForCdma();
        //foregroundCall.clearDisconnectedForCdma();
        //backgroundCall.clearDisconnectedForCdma();
    }
    ///////
  ...
  {
    if (this.ringingCall.getState() == CdmaCall.State.INCOMING)
      throw new CallStateException("cannot be in the incoming state");
    if (this.foregroundCall.getConnections().size() > 1)
    {
      if (DBG_POLL) Log.d("CDMA", "CP_COMM foregroundCall > 1");
      flashAndSetGenericTrue();
      return;
    }
    if (dcSize == 1)
    ...
    if (DBG_POLL) Log.d("CDMA", "CP_COMM hold() = " + mHoldNum);
    ...
    private void flashAndSetGenericTrue() throws CallStateException {
        ...
        {
            mHoldNum = 1;
            removeMessages(EVENT_CALLWAIT_TIMER);
        }
        if (DBG_POLL) log((new StringBuilder()).append("flashAndSetGenericTrue, mHoldNum = ").append(mHoldNum).toString());
        cm.switchWaitingOrHoldingAndActive(obtainCompleteMessage(EVENT_SWITCH_RESULT));//8
        ...
        switch (msg.what) {
            ...
            case EVENT_CALL_WAITING_INFO_CDMA:
               ...
               if (ar.exception == null) {
                   ...
                   /* //from 2.2
                    callwaitingNotification = (CdmaCallWaitingNotification)asyncresult.result;
                    pollCallsWhenSafe();
                    mRejectCWCall = false;
                    CWCallDc = null;
                   */
            ...
            case EVENT_THREE_WAY_DIAL_L2_RESULT_CDMA:
                ...
                if (ar.exception == null && pendingMO != null) {
                    ...
                    //pollCallsWhenSafe(); //from 2.2
                    pendingMO = null;
                }
            break;
            case EVENT_CALLWAIT_TIMER:
                if (DBG_POLL) log("[CdmaServiceStateTracker] EVENT_CALLWAIT_TIMER receive");
                if(mHoldNum == 0)
                    mRejectCWCall = true;
                break;
            case EVENT_CALL_FWIM_INFO_CDMA:
                if (DBG_POLL) Log.d("CDMA", "EVENT_CALL_FWIM_INFO_CDMA");
                CdmaConnection cdmaconnection = (CdmaConnection)(CdmaConnection)foregroundCall.getLatestConnection();
                if(cdmaconnection != null)
                {
                    cdmaconnection.updateconnetrealtime();
                    updatePhoneState();
                    fwimRegistrants.notifyRegistrants(new AsyncResult(null, null, null));
                }
                break;
            case EVENT_GPS_NOTIFY_IND:
                if (DBG_POLL) Log.d("CDMA", "[CdmaCallTracker] EVENT_GPS_NOTIFY_IND receive");
                //gpsRegistrants.notifyRegistrants(new AsyncResult(null, null, null));
                break;

            case EVENT_CALL_SECRET_INFO_CDMA:
                if (DBG_POLL) log("secret message has receive.");
                //secretRegistrants.notifyRegistrants((AsyncResult)message.obj);
                break;
