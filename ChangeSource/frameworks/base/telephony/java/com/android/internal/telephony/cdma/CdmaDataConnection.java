...
public class CdmaDataConnection extends DataConnection {
    ...
    //private static final String DialNumber = "#777"; //we can edit apns-conf, add protocal = "#777"
    /*
    see function         private ContentValues getRow(XmlPullParser parser) 
    ...
    */
    ...
    private void setHttpProxy(String s, String s1)
    {
        ...
        } else
        {
            if(s1 == null || s1.length() == 0)
                s1 = "8080";
            phone.setSystemProperty("net.gprs.http-proxy", (new StringBuilder()).append("http://").append(s).append(":").append(s1).append("/").toString());
        }
    }
    ...
    protected void onConnect(ConnectionParams cp) {
        ...
        if (DBG) log ((new StringBuilder()).append("Connecting to carrier: '").append(cp.apn.carrier).append("' APN: '").append(cp.apn.apn).append("' proxy: '").append(cp.apn.proxy).append("' port: '").append(cp.apn.port).toString());
