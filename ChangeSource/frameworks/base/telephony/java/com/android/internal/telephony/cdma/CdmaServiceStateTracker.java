...
final class CdmaServiceStateTracker extends ServiceStateTracker {
    ...
    private static final int NITZ_UPDATE_DIFF_DEFAULT = 10000;//2000;
    ...
    public CdmaServiceStateTracker(CDMAPhone phone) {
        ...
        cm.registerForRUIMPBMReady(this, EVENT_CARD_PBM_READY, null);
    ...
    public void handleMessage (Message msg) {
        ...
        case  EVENT_CARD_PBM_READY:
            cm.getCDMASubscription(obtainMessage(EVENT_POLL_STATE_CDMA_SUBSCRIPTION));
            if (DBG) log("Receive EVENT_CARD_PBM_READY and Send Request getCDMASubscription.");
            break;
        case EVENT_NV_READY:
            if (DBG) log("Receive EVENT_NV_READY and  getCDMASubscription ");
        ...
        case EVENT_GET_SIGNAL_STRENGTH:
			...
			{
                if (DBG) log("Receive EVENT_NETWORK_STATE_CHANGED_CDMA  cm.getRadioState().isRUIMReady() ");
		    }
            else if(mIsScreenOn)
            queueNextSignalStrengthPoll();
        ...
        case EVENT_SIGNAL_STRENGTH_UPDATE:
            ...
            onSignalStrengthResult(ar);
            if (DBG) log("Receive EVENT_SIGNAL_STRENGTH_UPDATE\t");
            break;
    ...
    protected void updateSpnDisplay() {
        ...
        if (operatorNumeric != null && !operatorNumeric.equals("")) {
            if(mSpnOverride.containsCarrier(operatorNumeric))
                operatorNumeric = mSpnOverride.getSpn(operatorNumeric);
            if (curSpnRule != 0
                    || !TextUtils.equals("", curSpn)
                    || !TextUtils.equals(operatorNumeric, curPlmn)) {
                phone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ALPHA_CDMA, operatorNumeric);
                Intent intent = new Intent(Intents.SPN_STRINGS_UPDATED_ACTION);
                //intent.addFlags(Intent.FLAG_RECEIVER_REPLACE_PENDING);
                intent.putExtra(Intents.EXTRA_SHOW_SPN, false);
                intent.putExtra(Intents.EXTRA_SPN, "");
                intent.putExtra(Intents.EXTRA_SHOW_PLMN, true);
                intent.putExtra(Intents.EXTRA_PLMN, operatorNumeric);
                phone.getContext().sendStickyBroadcast(intent);
            }

            curSpnRule = 0;//rule;
            curSpn = "";//spn;
            curPlmn = operatorNumeric;//plmn;
        }
    }
            ...
            case EVENT_POLL_STATE_OPERATOR_CDMA: // Handle RIL_REQUEST_OPERATOR
                ...
                if (opNames != null && opNames.length >= 3) {
                    ...
                    if (cm.getRadioState().isNVReady()) {
                        ...
                        opNames[0] = mSpnOverride.getSpn(opNames[2]);
                        opNames[1] = opNames[0];
    ...
    private void pollStateDone() {
        ...
        if (hasChanged) {
            ...
            updateSpnDisplay();
            if (DBG) Log.d("CDMA", (new StringBuilder()).append("phone. notifyServiceStateChanged").append(ss).toString());
    ...
    ///////////////+
    private void queueNextRegStatePoll()
    {
        if(mIsScreenOn && ss.getState() == 1)
        {
            if (DBG) log((new StringBuilder()).append("CDMA enter queueNextRegStatePoll() ").append(cm.getRadioState()).toString());
            removeMessages(41);
    ...
    private void
    queueNextSignalStrengthPoll() {
        if (dontPollSignalStrength && mSignalStrength.getCdmaDbm() != -115) //(cm.getRadioState().isGsm())) 
        {
            // The radio is telling us about signal strength changes
            // we don't have to ask it
            if (DBG) log("CDMA enter queueNextSignalStrengthPoll null ");
            return;
        }

        if (DBG) log("CDMA enter queueNextSignalStrengthPoll");
        Message msg;
    ...
    void setTimeFromNITZString (String nitz, long nitzReceiveTime)
    {
        ...
        if (DBG) Log.i(LOG_TAG, "NITZ: " + nitz + " ReceiveTime = " + nitzReceiveTime +
                        " HandleTime =" + start + " delay =" + (start - nitzReceiveTime));
        ...
        try {
            ...
            c.setTimeInMillis(c.getTimeInMillis() - 0x1b77400L); //28800000 = 8*60*60*1000
            ...
            String iso = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_CDMA, "cn");
            ...
            if (zone != null) {
                ...
                saveNitzTimeZone(zone.getID());
                if (DBG) Log.i("CDMA", (new StringBuilder()).append("NITZ: NetworkSetTimeZone = ").append(zone.getID()).toString());
            }
